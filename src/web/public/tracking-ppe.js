...
exportBtn?.addEventListener("click", (event) => {
  ...
  void downloadExportFile(`/api/depot/export?t=${Date.now()}`, "driver-ppe.xls").catch(
    (error) => {
...
function renderLive(depot, configuredTrucks) {
  ...
  if (!inDepot.length) {
    inDepotList.innerHTML = `<p class="empty-note">${
      query ? "No in-depot trucks match this search." : "No PPE trucks currently in the depot."
    }</p>`;
  } else {
...
async function refresh({ force = false } = {}) {
  ...
  hostLine.textContent = `http://${ip}${location.port ? `:${location.port}` : ""}/tracking/ppe`;
