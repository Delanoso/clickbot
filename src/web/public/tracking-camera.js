...
async function saveComment(truck, comment) {
  ...
  try {
    const res = await fetch(`/api/camera/trucks/${encodeURIComponent(truck)}/comment`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
...
async function removeTruck(truck) {
  ...
  try {
    const res = await fetch(`/api/camera/trucks/${encodeURIComponent(id)}`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({}),
    });
...
async function controlTask(action) {
  ...
  try {
    const res = await fetch(`/api/tasks/camera-monitor/${action}`, { method: "POST" });
    const data = await res.json().catch(() => ({}));
...
function renderFromCache({ forceWatchList = false } = {}) {
  renderLive(latestCamera, latestConfiguredTrucks);
  renderWatchList(latestConfiguredTrucks, latestLiveRows, { force: forceWatchList });
}

async function refreshLogs() {
  try {
    const res = await fetch("/api/tasks/camera-monitor/logs?tail=120");
...
async function refresh({ force = false } = {}) {
  ...
  hostLine.textContent = `http://${ip}${location.port ? `:${location.port}` : ""}/tracking/camera`;
  ...
  latestCamera = payload.camera || null;
